...
public class WebMainController {
    ...
    @Autowired
    private Product_AccountService product_accountService;
    ...
    public String index(Model model, @ModelAttribute("auth") Authority auth) throws IOException {
        try {
            ...
            // Map<Product, Account> product_accountMap = new HashMap<>();
            List<Product> products = cartSevice.findAll();
            model.addAttribute("products", products);
            // for (Product product : products) {
            //     Account accountStaff = product_accountService.getAccountWhoPostedProduct(product);
            //     product_accountMap.put(product, accountStaff);
            // }
            // model.addAttribute("product_accountMap", product_accountMap);
